    ...
    <form onSubmit={handleSearch} className="w-full max-w-2xl">
      <div className="flex gap-3 flex-col sm:flex-row">
        <select
          ...
          disabled={loading}
          className="px-4 py-3 bg-black border border-purple-500 rounded-lg text-yellow-300 focus:outline-none focus:ring-2 focus:ring-yellow-400 font-mono text-sm uppercase tracking-wider disabled:opacity-50 glow-border appearance-none bg-no-repeat pr-8"
          style={{
        ...
        <div className="flex gap-3 flex-1 glow-border p-1 rounded-lg bg-black/50 backdrop-blur-sm">
          <input
            type="text"
            placeholder={searchType === 'name' ? 'ENTER POKEMON NAME...' : 'ENTER ILLUSTRATOR NAME...'}
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            disabled={loading}
            className="flex-1 px-4 py-3 bg-black border-0 text-yellow-300 placeholder-purple-500/50 focus:outline-none font-mono text-sm uppercase tracking-wider disabled:opacity-50"
          />
          <button
            type="submit"
            disabled={loading}
            className="px-6 py-3 bg-gradient-to-r from-yellow-400 via-yellow-500 to-yellow-400 text-black font-black rounded-md hover:shadow-[0_0_20px_rgba(250,204,21,0.8)] transition-all duration-300 disabled:opacity-50 uppercase tracking-wider text-sm"
          >
